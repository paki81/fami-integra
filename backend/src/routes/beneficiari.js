...
router.post('/', authenticate, authorize('superadmin', 'admin', 'tutor', 'counselor'), [
  ...
  try {
    ...
    const fields = ['codice_id', 'cognome', 'nome', 'tipo_permesso', 'progetto_provenienza', 'nucleo_singolo', 'n_componenti_nucleo', 'area_intervento', 'comune', 'tipo_patente', 'automunito', 'latitudine', 'longitudine', 'budget_alloggio', 'note', 'data_uscita_sai', 'stato', 'competenze', 'nazionalita', 'livello_italiano', 'telefono', 'email', 'assegnato_a'];
  ...
  try {
    ...
    fields.forEach(f => {
      if (req.body[f] !== undefined) {
        ...
        if (val === '' && ['data_uscita_sai', 'n_componenti_nucleo', 'assegnato_a', 'latitudine', 'longitudine', 'budget_alloggio'].includes(f)) val = null;
        values.push(val);
      }
    });
